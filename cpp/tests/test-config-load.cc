...
TEST(TestConfigLoad, TestSampleConsumerParams)
{
	...
	EXPECT_EQ(
		...
		" sound; seg size: 1000 bytes; freshness: 0 ms; no device; 1 threads:\n"
		"[0: name: tiny; 30FPS; GOP: 30; Start bitrate: 100 Kbit/s; "
		"Max bitrate: 10000 Kbit/s; 320x180; Drop: NO]\n"
		"]\n",
		ss.str());
}
...
TEST(TestConfigLoad, TestSampleProducerParams)
{
	...
	EXPECT_EQ(
		...
		"username: clientA; prefix: /ndn/edu/ucla/remap;\n"
		"--0:\n"
		"stream source: /tmp/test-320x180-30frames.argb; session prefix: /ndn/edu/ucla/remap/ndnrtc/user/clientA; "
