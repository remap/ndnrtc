			...
			(const unsigned int w,const unsigned int h, unsigned char* data, unsigned int size)
		{
			...
			if (vp.get())
			{
				...
				else
				{
					nSkipped++;
#ifdef ENABLE_LOGGING
					LogDebug("") << "skipped " << frameName << " " << frameName[-1].toSequenceNumber() << std::endl;
#endif
				}
			}
			return 0;
				...
				queue.push([&queue, &cache, i, buffer, &onDataArrived](){
					...
					cache.addInterest(i, [&queue, buffer, &onDataArrived](const boost::shared_ptr<ndn::Data>& d){
						...
						queue.push([buffer, d, &onDataArrived](){
							...
							if (info.isParity_)
								data = boost::make_shared<WireData<DataSegmentHeader>>(d);
							else
								data = boost::make_shared<WireData<VideoFrameSegmentHeader>>(d);
