...
public:
	MOCK_METHOD2(onStateMachineChangedState, void(const std::shared_ptr<const ndnrtc::PipelineControlEvent>&,
			std::string newState));
	MOCK_METHOD2(onStateMachineReceivedEvent, void(const std::shared_ptr<const ndnrtc::PipelineControlEvent>&,
			std::string state));
};
